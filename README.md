...
Beide Walls sind Single-User-Prozesse und setzen Cognees interne Multi-Tenant-ACL ausdrücklich aus. Die Datenschutzgrenze bleibt physisch pro Wall; innerhalb einer Wall begrenzen Cognee-Datasets den Abruf und `kb` erzwingt anschließend eine vollständige Vault-/Source-Allowlist. Bekannte Treffer aus einem anderen Vault werden verworfen, ohne sie als Wissenslücke auszugeben. `kb restart` beendet die gesamte Dienst-Prozessgruppe, damit Ladybug- und LanceDB-Kinder keinen Store-Lock zurücklassen.
