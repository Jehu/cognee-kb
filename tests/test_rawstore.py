...
def test_slugify():
    assert (
        slugify("Künstliche Intelligenz: Ein Überblick!")
        == "kuenstliche-intelligenz-ein-ueberblick"
    )
...
def test_write_raw_creates_file_with_frontmatter(tmp_path):
    r = SourceRecord.new(
        type="snippet", url=None, video_id=None, locator=None, vault="privat", raw_md_path=""
    )
